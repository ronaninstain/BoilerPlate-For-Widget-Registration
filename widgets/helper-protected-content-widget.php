...
class Helper_Protected_Content_Widget extends \Elementor\Widget_Base
{
	...
	{
		return __('Protected Content', 'pc');
	}
	...
	function register_content_controls()
	{
		...
		$this->add_control(
			'password',
			[
				'label'       => __('Password', 'pc'),
				'type'        => \Elementor\Controls_Manager::TEXT,
				'input_type'  => 'text',
				'placeholder' => __('Password', 'pc'),
				'label_block' => true
			]
		);
		...
		$this->add_control(
			...
			[
				...
				'placeholder' => __('Non Protected Message', 'pc'),
			]
		);
		...
		$this->add_control(
			'protected_message',
			[
				'label'       => __('Protected Message', 'pc'),
				'type'        => \Elementor\Controls_Manager::TEXTAREA,
				'placeholder' => __('Protected Message', 'pc'),
			]
		);
...
?>
			<p <?php echo $this->get_render_attribute_string('message') ?>> <?php echo esc_html($dummy_text); ?></p>
			<div>
				<form action="<?php the_permalink(); ?>" method="POST">
					<input type="hidden" name="ph" value="<?php echo md5($password); ?>">
					<label for="">Input Password</label><br />
					<input type="password" class="passwrd"><br />
